...
def extractive_fallback(context: str, question: str, return_confidence: bool = False) -> str | tuple[str, float]:
    """
    Extract best matching sentence from context using scoring.
    ...
    if not sentences:
        if return_confidence:
            return "I don't know from the provided context.", 0.0
    ...
    # Normalize confidence to 0-1 range (heuristic: score >= 3 is high confidence)
    confidence = min(best_score / 5.0, 1.0)
    
    if return_confidence:
        return best_sentence.strip(), confidence
...
def generate_from_model(
    ...
    config: dict,
    prompt: str,
    max_new_tokens: int = 12,  # Increased from 8 to 12 for factual QA
) -> str:
    """
    Generate answer using greedy decoding (no sampling).
    
    Greedy decoding ensures deterministic, concise factual extraction.
    Max tokens limited to prevent runaway generation.
    """
    ...
    for _ in range(max_new_tokens):
        ...
        # Greedy decoding: always take highest probability token
        # No sampling, no temperature - ensures deterministic factual extraction
...
def generate_answer(
    ...
    model=None,
    use_extractive_first: bool = False,
) -> str:
    """
    Generate answer using fine-tuned GPT-2 with optional extractive shortcut.
    
    Args:
        context: Text context
        question: Question to answer
        use_extractive_first: If True, try extractive answering before generation
            (major latency optimization for factual QA)
    
    Returns:
        Generated answer
    """
    # Extractive shortcut for factual questions - try extraction first
    if use_extractive_first:
        extractive_answer, confidence = extractive_fallback(context, question, return_confidence=True)
        # High confidence threshold: 0.6 or above means we trust the extractive answer
        if confidence >= 0.6:
            return extractive_answer
        # Low confidence: fall through to LLM generation
